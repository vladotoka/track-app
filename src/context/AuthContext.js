	...
	async ({ email, password }) => {
		try {
			...
			if (/Path `email` is required/.test(err.response.data)) {
				...
				/E11000 duplicate key error collection:/.test(err.response.data)
			) {
				errMsg = 'Email is already used';
			} else if (err.response.data === 404) {
				errMsg =
					'This request returns an error 404, check if localtunel is running or change the axios baseUrl in src/api/tracker';
			} else {
				errMsg = 'Something went wrong with sign up';
			}
	...
	async ({ email, password }) => {
		try {
			...
			if (err.response.data.error) {
				...
				errMsg = 'Something went wrong with sign in';
			}
